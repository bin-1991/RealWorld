...
class HexEditWidgetPrivate : public EditorService
{
public:
    HexEditWidgetPrivate(HexEdit *widget) : q(widget) {}
    ~HexEditWidgetPrivate() override { if (m_aboutToBeDestroyedHandler) m_aboutToBeDestroyedHandler(); }
...
HexEdit::HexEdit(QWidget *parent)
    : QAbstractScrollArea(parent), d(new HexEditWidgetPrivate(this))
{
    m_bytesPerLine = 16;
    m_ieditor = nullptr;
    m_baseAddr = 0;
...
EditorService *HexEdit::editorService() const
{
    return d;
}
...
void HexEdit::addData(quint64 addr, const QByteArray &data)
{
...
bool HexEdit::requestDataAt(qint64 pos) const
{
...
bool HexEdit::requestOldDataAt(qint64 pos) const
{
    qint64 block = pos / m_blockSize;
...
char HexEdit::dataAt(qint64 pos, bool old) const
{
    qint64 block = pos / m_blockSize;
...
void HexEdit::changeDataAt(qint64 pos, char c)
{
    qint64 block = pos / m_blockSize;
...
QByteArray HexEdit::dataMid(qint64 from, int length, bool old) const
{
...
QByteArray HexEdit::blockData(qint64 block, bool old) const
{
    if (old) {
        BlockMap::const_iterator it = m_modifiedData.find(block);
...
void HexEdit::setBlinkingCursorEnabled(bool enable)
{
...
void HexEdit::focusInEvent(QFocusEvent *)
{
    setBlinkingCursorEnabled(true);
}

void HexEdit::focusOutEvent(QFocusEvent *)
{
    setBlinkingCursorEnabled(false);
}

void HexEdit::timerEvent(QTimerEvent *e)
{
...
void HexEdit::setModified(bool modified)
{
...
bool HexEdit::isReadOnly() const
{
    return m_readOnly;
}

bool HexEdit::save(QString *errorString, const QString &oldFileName, const QString &newFileName)
{
...
void HexEdit::setSizes(quint64 startAddr, qint64 range, int blockSize)
{
...
void HexEdit::changeEvent(QEvent *e)
{
...
QRect HexEdit::cursorRect() const
{
...
bool HexEdit::inTextArea(const QPoint &pos) const
{
    int xoffset = horizontalScrollBar()->value();
    int x = xoffset + pos.x() - m_margin - m_labelWidth;
...
void HexEdit::updateLines()
{
    updateLines(m_cursorPosition, m_cursorPosition);
}

void HexEdit::updateLines(int fromPosition, int toPosition)
{
    int topLine = verticalScrollBar()->value();
...
int HexEdit::dataIndexOf(const QByteArray &pattern, qint64 from, bool caseSensitive) const
{
...
int HexEdit::dataLastIndexOf(const QByteArray &pattern, qint64 from, bool caseSensitive) const
{
    int trailing = pattern.size();
    if (trailing > m_blockSize)
        return -1;
...
int HexEdit::find(const QByteArray &pattern_arg, qint64 from,
...
int HexEdit::findPattern(const QByteArray &data, const QByteArray &dataHex,
...
void HexEdit::drawItems(QPainter *painter, int x, int y, const QString &itemString)
{
...
void HexEdit::drawChanges(QPainter *painter, int x, int y, const char *changes)
{
...
QString HexEdit::addressString(quint64 address)
{
...
void HexEdit::paintEvent(QPaintEvent *e)
{
...
qint64 HexEdit::cursorPosition() const
{
    return m_cursorPosition;
}

void HexEdit::setCursorPosition(qint64 pos, MoveMode moveMode)
{
...
void HexEdit::mouseMoveEvent(QMouseEvent *e)
{
...
void HexEdit::mouseReleaseEvent(QMouseEvent *)
{
...
QString HexEdit::toolTip(const QHelpEvent *helpEvent) const
{
...
void HexEdit::zoomF(float delta)
{
...
void HexEdit::highlightSearchResults(const QByteArray &pattern, QTextDocument::FindFlags findFlags)
{
...
void HexEdit::changeData(int position, uchar character, bool highNibble)
{
...
void HexEdit::setupJumpToMenuAction(QMenu *menu, QAction *actionHere,
...
void HexEdit::jumpToAddress(quint64 address)
{
...
void HexEdit::setNewWindowRequestAllowed(bool c)
{
    m_canRequestNewWindow = c;
}

void HexEdit::updateContents()
{
    m_oldData = m_data;
    m_data.clear();
    m_modifiedData.clear();
...
QPoint HexEdit::offsetToPos(qint64 offset) const
{
...
void HexEdit::asFloat(qint64 offset, float &value, bool old) const
{
    value = 0;
...
void HexEdit::asDouble(qint64 offset, double &value, bool old) const
{
    value = 0;
...
void HexEdit::asIntegers(qint64 offset, int count, quint64 &bigEndianValue,
...
void HexEdit::setMarkup(const QList<Markup> &markup)
{
    m_markup = markup;
    viewport()->update();
}
